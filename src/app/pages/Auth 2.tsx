...
export function Auth() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      localStorage.setItem("exact_subscription", data.user.subscription_status);

      // If new account without subscription, redirect to checkout
  ...
  return (
    <div className="min-h-screen bg-[#F2F0EB] text-black font-sans selection:bg-[#0000FF] selection:text-white flex flex-col items-center justify-center p-4 md:p-8">
      ...
      <div
        className="absolute top-6 left-6 flex items-center gap-2 cursor-pointer hover:opacity-70 transition-opacity"
        onClick={() => navigate('/')}
      >
      ...
      <div className="w-full max-w-md bg-white border-[2px] border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] p-8 md:p-10 relative">
        ...
        {error && (
          <div className="bg-red-50 border-[2px] border-red-500 text-red-700 px-4 py-3 mb-6 font-secondary text-sm">
            {error}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-5 flex flex-col">
          ...
          <div className="space-y-2">
            ...
            />
            {!isLogin && (
              <p className="text-xs font-secondary text-gray-500">Minimum 8 caracteres</p>
            )}
          </div>
